...
// First parts identical to Example 4.6 and Exercise 4.7
...
impl Expr {
    ...
    pub fn good_env(&self, env: &Environment) -> bool {
        #[pure]
        fn helper(args: &List<Expr>, env: &Environment) -> bool {
            match args {
                Nil => true,
                Cons(h, t) => h.good_env(env) && helper(t, env),
            }
        }
        match self {
            Const(_) => true,
            Var(s) => env.contains_key(s), // FUTURE: map_support, std_lib_extern_spec_enhancement
            Node(_, args) => helper(args, env),
        }
    }
}
